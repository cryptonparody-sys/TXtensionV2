...
class DiscordReplyController {
  constructor(config, storageKey) {
    ...
    this.boundReposition = this.repositionOverlays.bind(this);
    this.rtlLanguages = new Set(config.rtlLanguages || []);
    this.debugDiscordMessages = false; // Set to true for debugging Discord message detection
    this.selectors = {
  ...
  extractMessage(message) {
    ...
    // Find the main message content, excluding replied-to message previews
    const mainMessageContent = this.findMainMessageContent(message);
    if (mainMessageContent) {
      const text = mainMessageContent.innerText?.trim();
      if (text) segments.push(text);
    }
  ...
  // Find only the main message content, excluding replied-to previews
  findMainMessageContent(message) {
    // Try multiple strategies to find the main message content
    if (this.debugDiscordMessages) {
      console.log('[Xtension] Discord: Finding main message content...');
    }

    // Strategy 1: Look for the direct message-content element that's not inside a reply preview
    const allMessageContents = message.querySelectorAll('[id^="message-content"]');
    if (this.debugDiscordMessages) {
      console.log(`[Xtension] Discord: Found ${allMessageContents.length} message content elements`);
    }

    for (const content of allMessageContents) {
      const isReplyPreview = this.isInsideReplyPreview(content);
      if (this.debugDiscordMessages) {
        console.log(`[Xtension] Discord: Message content "${content.innerText?.substring(0, 50)}..." is reply preview: ${isReplyPreview}`);
      }

      // Skip if this content is inside a replied-to message preview
      if (isReplyPreview) {
        continue;
      }
      if (this.debugDiscordMessages) {
        console.log(`[Xtension] Discord: Selected main message content: "${content.innerText?.substring(0, 50)}..."`);
      }
      return content;
    }

    // Strategy 2: Look for message content that's a direct child of the main message container
    const directContent = message.querySelector(':scope > [id^="message-content"], :scope > div > [id^="message-content"]');
    if (directContent && !this.isInsideReplyPreview(directContent)) {
      if (this.debugDiscordMessages) {
        console.log(`[Xtension] Discord: Strategy 2 selected: "${directContent.innerText?.substring(0, 50)}..."`);
      }
      return directContent;
    }

    // Strategy 3: Look for the first message content that's not in a compact/dimmed container (typical for reply previews)
    const firstNormalContent = Array.from(allMessageContents).find(content => {
      const parent = content.closest('[class*="compact"], [class*="replied"], [class*="preview"]');
      return !parent;
    });
    if (firstNormalContent) {
      if (this.debugDiscordMessages) {
        console.log(`[Xtension] Discord: Strategy 3 selected: "${firstNormalContent.innerText?.substring(0, 50)}..."`);
      }
      return firstNormalContent;
    }

    // Strategy 4: Last resort - return the last message content (usually the main message)
    if (allMessageContents.length > 0) {
      const lastContent = allMessageContents[allMessageContents.length - 1];
      if (this.debugDiscordMessages) {
        console.log(`[Xtension] Discord: Strategy 4 (last resort) selected: "${lastContent.innerText?.substring(0, 50)}..."`);
      }
      return lastContent;
    }

    if (this.debugDiscordMessages) {
      console.log('[Xtension] Discord: No message content found');
    }
    return null;
  }

  // Check if an element is inside a replied-to message preview
  isInsideReplyPreview(element) {
    // Check if the element is inside containers that typically indicate reply previews
    const replyPreviewSelectors = [
      '[class*="replied"]',
      '[class*="replying"]',
      '[class*="preview"]',
      '[class*="compact"]',
    ...
    for (const selector of replyPreviewSelectors) {
      const parent = element.closest(selector);
      if (parent) {
        if (this.debugDiscordMessages) {
          console.log(`[Xtension] Discord: Found reply preview via selector "${selector}"`);
        }
        return true;
      }
    }

    // Additional check: Look for elements with opacity less than 0.7 (common for reply previews)
    let currentElement = element.parentElement;
    while (currentElement && currentElement !== element) {
      const computedStyle = window.getComputedStyle(currentElement);
      if (computedStyle.opacity && parseFloat(computedStyle.opacity) < 0.7) {
        // Check if this looks like a reply preview (smaller text, muted colors)
        const fontSize = parseFloat(computedStyle.fontSize);
        if (fontSize < 13) { // Reply previews usually have smaller font
          if (this.debugDiscordMessages) {
            console.log(`[Xtension] Discord: Found reply preview via opacity (${computedStyle.opacity}) and font-size (${fontSize}px)`);
          }
          return true;
        }
      }
    ...
    if (this.debugDiscordMessages) {
      console.log(`[Xtension] Discord: Element is not inside reply preview`);
    }
    return false;
  }
